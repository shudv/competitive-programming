...
typedef struct { int from; int to; int capacity; int flow; } edge; 
vvi G(505);
vector<edge> edges;
...
int binary_search(const vi& values, int value, int i, int j)
{
    ...
    if (values[mid] > value) return binary_search(values, value, i+1, mid-1);
...
int main() 
{
    ...
    while(true) 
    {
        ...
        typedef struct { int amount; int start; int end; } drinking_slot; 
